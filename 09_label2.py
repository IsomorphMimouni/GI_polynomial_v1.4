...
print(100*"~")
# les entetes:
cur.execute("select distinct tete from arbre2")
ligne = cur.fetchall()
list_tete=[]
for t in ligne:
...
for tete in list_tete:
	#max niveau:
	cur.execute("select max(niveau) from arbre2 where tete=?", (tete,))
	ligne = cur.fetchone()
	max=ligne[0]
	#print("max=",max)
	cur.execute("select sommets from arbre2 where niveau=0 and tete=?", (tete,))
	ligne = cur.fetchone()
	sommet=ligne[0]
	#niveau0
	cur.execute("select sommets from arbre2 where niveau=1 and tete=?", (tete,))
	ligne = cur.fetchone()
	sommets=ligne[0]
	...
	#insertion
	cur.execute("insert into labels2 (sommet, etiquette) values (?, ?)", (sommet, label))#il y a des doublon
	...
	while(niv<max):
		...
		niv_bas=niv+1
		#haut:
		cur.execute("select sommets from arbre2 where niveau=? and tete=?", (niv_haut, tete))
		ligne = cur.fetchone()
		sommets_h=ligne[0]
		list_sommets_h=sommets_h.split(',')
		set_sommets_h=set(list_sommets_h)
		#horizon
		cur.execute("select sommets from arbre2 where niveau=? and tete=?", (niv, tete))
		ligne = cur.fetchone()
		sommets_h=ligne[0]
		list_sommets_m=sommets_h.split(',')
		set_sommets_m=set(list_sommets_m)
		#bas:
		cur.execute("select sommets from arbre2 where niveau=? and tete=?", (niv_bas, tete))
		ligne = cur.fetchone()
		sommets_b=ligne[0]
